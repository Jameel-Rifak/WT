...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($eventDate < $today) {
        ...
        if ($_FILES["image"]["name"]) {
            $imageDir = __DIR__ . "/uploads/"; // Absolute path to uploads directory
            $imageName = time() . "_" . $_FILES["image"]["name"]; // Unique filename to avoid conflicts
            $imagePath = $imageDir . $imageName;

            // Check if the uploads directory exists, if not, create it
            if (!file_exists($imageDir)) {
                mkdir($imageDir, 0755, true); // Create the uploads directory with appropriate permissions
            }
